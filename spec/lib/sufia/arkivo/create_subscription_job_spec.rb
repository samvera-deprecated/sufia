...
describe Sufia::Arkivo::CreateSubscriptionJob do
  ...
  context 'with a bogus user' do
    ...
    it 'raises because user not found' do
      expect { subject.call }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User not found')
    end
  end
  ...
  context 'without an arkivo token' do
    ...
    it 'raises because user lacks arkivo token' do
      expect { subject.call }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User does not have an Arkivo token')
    end
  end

  context 'without a zotero userid' do
    it 'raises because user did not oauth' do
      expect { subject.call }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User has not yet connected with Zotero')
    end
  end
  ...
  context 'with an existing subscription' do
    ...
    it 'raises because user already has subscription' do
      expect { subject.call }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User already has a subscription')
    end
  end
  ...
  context 'when expected to succeed' do
    ...
    it 'stores a subscription URL for possible later invalidation' do
      ...
      subject.call
      expect(user.reload.arkivo_subscription).to match %r{/api/subscription/.{9,}}
    end
  end
end
