...
describe User, type: :model do
  ...
  describe "trophy_works" do
    let(:user) { FactoryGirl.create(:user) }
    let(:work1) { Work.create(title: ["work A"]) { |w| w.apply_depositor_metadata(user) } }
    let(:work2) { Work.create(title: ["work B"]) { |w| w.apply_depositor_metadata(user) } }
    let(:work3) { Work.create(title: ["work C"]) { |w| w.apply_depositor_metadata(user) } }
