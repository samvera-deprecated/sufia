...
describe FileSetAttachedEventJob do
  ...
  context 'with a FileSet' do
    ...
    let(:event) do
      {
        action: "User <a href=\"/users/#{user.to_param}\">#{user.user_key}</a> has attached <a href=\"/concern/file_sets/#{file_set.id}\">A Contained FileSet</a> to <a href=\"/concern/works/#{curation_concern.id}\">MacBeth</a>",
        timestamp: '1'
      }
    end
